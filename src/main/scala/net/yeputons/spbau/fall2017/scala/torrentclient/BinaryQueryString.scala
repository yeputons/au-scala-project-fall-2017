...
class BinaryQueryString(args: Seq[String]) {
  def rawQueryString: Option[String] = if (args.isEmpty) None else Some(args.mkString("&"))

  def +[K, V](kv: (K, V))(implicit encK: BinaryQueryEncoder[K], encV: BinaryQueryEncoder[V]): BinaryQueryString = {
...
object BinaryQueryString {
  def apply(rawQueryString: Option[String]): BinaryQueryString = rawQueryString match {
...
object BinaryQueryEncoder {
  ...
  implicit object ArrayByteEncoder extends BinaryQueryEncoder[Array[Byte]] {
    override def apply(v: Array[Byte]): String = v.map(x => f"${x & 0xFF}%02x").mkString
  }
}