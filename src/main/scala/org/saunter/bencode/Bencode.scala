 ...
 */
// Obtained from https://github.com/danluu/storrent/blob/master/src/main/scala/Bencode.scala
...
object BencodeEncoder {
  ...
  def encode(input: Any): String =
    input match {
      ...
      case x: Map[_, _] => dictionary(x.asInstanceOf[Map[String, _]])  // Cannot be checked due to type erasure
      case _ => ""
    }
